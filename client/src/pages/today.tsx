...
const Today: React.FC<TodayProps> = ({ userId }) => {
  ...
  const activeCycleForSelectedDate = useMemo(() => {
    ...
    return cycles.find((cycle) => {
      ...
      if (cycle.endDate && isSameDay(selectedDate, parseISO(cycle.endDate)))
        return true;
      ...
      const isBeforeEnd =
        !cycle.endDate || isBefore(selectedDate, parseISO(cycle.endDate));
  ...
  const cycleAnalysis = useMemo(() => {
    ...
    const periodStarts = periodRecords.reduce((acc, record, index) => {
      ...
      if (
        index === 0 ||
        differenceInDays(currentDate, parseISO(periodRecords[index - 1].date)) >
          2
      ) {
    ...
    const { avgCycleLength, avgPeriodLength } = getBestCyclePredictionLengths(
      flowRecords,
      userSettings
    );
    ...
    const lastPeriodStart = periodStarts
      .filter((start) => !isAfter(start, selectedDate))
      .pop();
    ...
    // --- Phase Calculation ---
    let phase: "period" | "follicular" | "ovulation" | "luteal" | "Unknown" =
      "Unknown";
    // Use currentFlow for *actual* period status on the day
    if (currentFlow && currentFlow.intensity !== "spotting") {
      phase = "period";
    } else if (isPredicted && !activeCycleForSelectedDate) {
      // Show predicted *only* if no actual period cycle active
    ...
    return {
      ...
      isPredictedPeriod:
        isPredicted &&
        phase === "period" &&
        !(currentFlow && currentFlow.intensity !== "spotting"), // Only true if predicted AND no actual flow logged
      dayOfCycle,
    };
  }, [
    flowRecords,
  ...
  // --- START: REVISED handleStartPeriod ---
  const handleStartPeriod = useCallback(() => {
    // The startPeriod function from useCycleData now handles
    // creating the cycle, auto-logging light flow, and refetching.
    // We just need to call it.
    // You can add 'await' if startPeriod itself returns a promise you need to wait for,
    // but often mutations handle their own async logic.
    startPeriod(selectedDate);

    // Note: Error handling and success toasts are typically managed within the
    // useMutation definitions inside the useCycleData hook.
    // You could add a try/catch here if startPeriod could throw an
    // error *before* the mutation runs, but it's usually not necessary.

    // Dependencies: Only include what's directly used in *this* callback.
  }, [startPeriod, selectedDate]);
  // --- END: REVISED handleStartPeriod ---
  ...
  const canEditFlow =
    ...
    isSameDay(selectedDate, parseISO(activeCycleForSelectedDate.endDate)); // Allow editing on end date
    ...
    <div className="tab-content px-4 py-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-4">
          {isToday(selectedDate) ? "Today" : "Day View"}
        </h2>
        <Card>
          <CardContent className="p-4">
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              ...
              {cycleAnalysis.phase !== "Unknown" && (
                <div
                  className={`text-sm px-3 py-1 rounded-full text-white period-status-label
                                    ${
                                      cycleAnalysis.phase === "period"
                                        ? cycleAnalysis.isPredictedPeriod
                                          ? "bg-red-300"
                                          : "bg-red-500"
                                        : cycleAnalysis.phase === "follicular"
                                        ? "bg-yellow-500"
                                        : cycleAnalysis.phase === "ovulation"
                                        ? "bg-blue-500"
                                        : cycleAnalysis.phase === "luteal"
                                        ? "bg-purple-500"
                ...
                >
                  {cycleAnalysis.isPredictedPeriod
                    ? "Predicted Period"
                    : `${
                        cycleAnalysis.phase.charAt(0).toUpperCase() +
                        cycleAnalysis.phase.slice(1)
                      } Phase`}
                </div>
              )}
            ...
            {cycleAnalysis.phase !== "Unknown" &&
              !cycleAnalysis.isPredictedPeriod && (
                <div className="mb-4 p-3 rounded-md text-sm bg-muted/40">
                  {cycleAnalysis.phase === "period" && (
                    <p>
                      Menstrual phase: Your period is logged for this day.
                      Hormone levels are typically at their lowest.
                    </p>
                  )}
                  {cycleAnalysis.phase === "follicular" && (
                    <p>
                      Follicular phase: Began after your last period and lasts
                      until ovulation. Estrogen rises.
                    </p>
                  )}
                  {cycleAnalysis.phase === "ovulation" && (
                    <p>
                      Ovulation phase: An egg is released. This is typically
                      your most fertile time.
                    </p>
                  )}
                  {cycleAnalysis.phase === "luteal" && (
                    <p>
                      Luteal phase: After ovulation, progesterone rises,
                      preparing for potential pregnancy or the next period.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="mb-4">
                ...
                <div className="flex gap-2">
                  ...
                  {!activeCycleForSelectedDate && (
                    <>
                      <Button
                        variant={
                          currentFlow?.intensity === "spotting"
                            ? "default"
                            : "outline"
                        }
                        size="sm"
                        className={`text-xs flex items-center gap-1 px-3 py-1.5 period-status-btn border-primary ${
                          currentFlow?.intensity === "spotting"
                      ...
                      >
                        <DropletIcon
                          className="h-4 w-4"
                          fillOpacity={
                            currentFlow?.intensity === "spotting" ? 0.3 : 0
                          }
                        />
                        Spotting
                      </Button>
                    ...
                    <>
                      ...
                      {!activeCycleForSelectedDate.endDate && (
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogDescription>
                              End your period on{" "}
                              {format(selectedDate, "MMMM d, yyyy")}? This will
                              finalize the cycle length.
                            </AlertDialogDescription>
                            <AlertDialogFooter>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          ...
                          <AlertDialogDescription>
                            Permanently remove this period cycle (
                            {format(
                              parseISO(activeCycleForSelectedDate.startDate),
                              "MMM d"
                            )}{" "}
                            -{" "}
                            {activeCycleForSelectedDate.endDate
                              ? format(
                                  parseISO(activeCycleForSelectedDate.endDate),
                                  "MMM d"
                                )
                              : "Ongoing"}
                            ) and all its associated flow data?
                          </AlertDialogDescription>
                          <AlertDialogFooter>
                            <AlertDialogCancel>No, Keep It</AlertDialogCancel>
                            <AlertDialogAction onClick={handleCancelPeriod}>
                              Yes, Delete It
                            </AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
              ...
              {/* ---- START: Flow Intensity / Status Section ---- */}

              {/* Show this whole section ONLY if a cycle exists for the date */}
              {activeCycleForSelectedDate && (
                <div className="flex flex-col items-center mb-3">
                  {/* Tooltip/Info for past log days */}
                      ...
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent side="bottom">
                            {currentFlow.intensity !== "spotting"
                              ? "This flow was logged for a past period. You can edit it."
                              : "Spotting was logged for this past period day."}
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
                  ...
                  {/* Render Flow Intensity Buttons IF:
                                        1. The cycle is ongoing OR
                                        2. The cycle is ended AND non-spotting flow was logged for *this* day
                                    */}
                  {(!activeCycleForSelectedDate.endDate ||
                    (activeCycleForSelectedDate.endDate &&
                      currentFlow &&
                    ...
                    <div className="flex gap-3 justify-center">
                      <Button
                        variant={
                          currentFlow?.intensity === "light"
                            ? "default"
                            : "outline"
                        }
                        className={`period-status-btn flex items-center justify-center px-4 py-2 ${
                          currentFlow?.intensity === "light"
                            ? "bg-primary selected"
                      ...
                      >
                        <span className="flex gap-1 items-center justify-center">
                          <DropletIcon
                            className="h-5 w-5"
                            fillOpacity={
                              currentFlow?.intensity === "light" ? 0.5 : 0
                            }
                          />
                        </span>
                      </Button>
                      ...
                      >
                        <span className="flex gap-1 items-center justify-center">
                          <DropletIcon
                            className="h-5 w-5"
                            fillOpacity={
                              currentFlow?.intensity === "medium" ? 0.5 : 0
                            }
                          />
                          <DropletIcon
                            className="h-5 w-5"
                      ...
                      >
                        <span className="flex gap-1 items-center justify-center">
                          <DropletIcon
                            className="h-5 w-5"
                            fillOpacity={
                              currentFlow?.intensity === "heavy" ? 0.5 : 0
                            }
                          />
                          <DropletIcon
                            className="h-5 w-5"
                  ...
                  {activeCycleForSelectedDate.endDate &&
                    (!currentFlow || currentFlow.intensity === "spotting") && (
                      <div className="text-sm text-muted-foreground text-center mt-2">
                        {currentFlow?.intensity === "spotting"
                          ? "Spotting was logged for this past day."
                          : "No flow was logged for this past day."}
                      </div>
                    )}
                </div>
              )}

              {/* Show helper text ONLY if NO cycle is active for the date */}
              {/* ---- START: Modified Helper Text Logic ---- */}
              {!activeCycleForSelectedDate && (
                <div className="text-sm text-muted-foreground mb-3 p-2 bg-muted/20 rounded flex items-center justify-center gap-2 min-h-[40px]">
                  {" "}
                  {/* Added flex, gap, min-h */}
                  {currentFlow && currentFlow.intensity === "spotting" ? (
                    <>
                      <DropletIcon
                    ...
                    <span>
                      Log spotting or use the "Start Period" button to begin
                      tracking.
                    </span>
                  )}
                </div>
              )}
              {/* ---- END: Modified Helper Text Logic ---- */}
              {/* ---- END: Flow Intensity / Status Section ---- */}
            </div>
            ...
            <div className="mb-6">
              ...
              <SymptomsList
                ...
                activeSymptomIds={physicalSymptoms
                  .filter((s: any) => isSymptomActive(s.id))
                  .map((s: any) => s.id)}
            ...
            <div className="mb-6">
              ...
              <SymptomsList
                ...
                activeSymptomIds={emotionalSymptoms
                  .filter((s: any) => isSymptomActive(s.id))
                  .map((s: any) => s.id)}
                onToggleSymptom={toggleSymptom}
                getSymptomIntensity={getSymptomIntensity}
            ...
            {userSettings?.showPmddSymptoms &&
              pmddSymptoms &&
              pmddSymptoms.length > 0 && (
                <div className="mb-6">
                  ...
                  <div className="p-3 mb-3 bg-muted/20 rounded-md text-sm text-muted-foreground">
                    <p>
                      Track symptoms typically severe during the luteal phase
                      (before period) and improving shortly after menstruation
                      starts.
                    </p>
                  </div>
                  <SymptomsList
                    symptoms={pmddSymptoms}
                    activeSymptomIds={pmddSymptoms
                      .filter((s: any) => isSymptomActive(s.id))
                      .map((s: any) => s.id)}
                    onToggleSymptom={toggleSymptom}
                    getSymptomIntensity={getSymptomIntensity}
